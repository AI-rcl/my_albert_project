...
BERT_BASE_DIR=os.path.join(os.path.dirname(__file__),'albert_tiny_zh')
...
model_path=os.path.join(bot_path,'model')
# label_list=sys.argv[-3].split(',')
...
flags.DEFINE_integer("train_batch_size", 28, "Total batch size for training.")
...
flags.DEFINE_float("num_train_epochs", train_epoch,
...
class SimProcessor(DataProcessor):
    ...
    def get_labels(self,data_dir):
        ...
        dev_file_path=os.path.join(data_dir,'test.tsv')
        train_file = open(train_file_path, 'r', encoding='utf-8')
        dev_file=open(dev_file_path,'r',encoding='utf-8')
        label_list = []
        for train_line in train_file.readlines():
            train_line = train_line.replace("\n", "").split("\t")
            train_label=str(train_line[0])
            label_list.append(train_label)
        for dev_line in dev_file.readlines():
            dev_line=dev_line.replace("\n", "").split("\t")
            dev_label=str(dev_line[0])
            label_list.append(dev_label)
